...
### Added / 추가
- Root() 함수 구현 - 에러 체인의 루트(가장 안쪽) 에러 반환
- UnwrapAll() 함수 구현 - 에러 체인의 모든 에러를 슬라이스로 반환
- Contains() 함수 구현 - 에러 체인이 특정 에러를 포함하는지 확인

### Changed / 변경
- 테스트 커버리지 99.2% → 99.3%로 증가

### Files Changed / 변경된 파일
- `cfg/app.yaml` - 버전을 v1.12.016에서 v1.12.017로 증가
- `errorutil/inspect.go` - Root(), UnwrapAll(), Contains() 함수 추가 (115줄)
- `errorutil/inspect_test.go` - 새 함수에 대한 테스트 추가 (167줄)
- `docs/CHANGELOG/CHANGELOG-v1.12.md` - v1.12.017 항목 추가
...
**Why / 이유**:
- WORK_PLAN Phase 4에 계획되었으나 미구현된 유용한 함수들
- 에러 체인 탐색 및 검사 기능 강화
- 에러 디버깅 및 분석 시 유용한 유틸리티

**Implementation Details / 구현 세부사항**:

1. **Root() 함수**:
   - 에러 체인을 끝까지 언래핑하여 원본 에러 반환
   - 디버깅 시 원인 에러를 빠르게 파악 가능
   - 다중 레이어 래핑된 에러의 근본 원인 추적

2. **UnwrapAll() 함수**:
   - 에러 체인의 모든 에러를 슬라이스로 반환
   - 첫 번째 요소는 가장 바깥쪽, 마지막은 루트 에러
   - 전체 에러 컨텍스트 파악 및 로깅에 유용

3. **Contains() 함수**:
   - errors.Is()를 래핑한 편의 함수
   - 에러 체인에서 특정 센티널 에러 검색
   - 에러 타입 기반 처리 로직에 활용

4. **테스트**:
   - 각 함수당 4-6개 테스트 케이스
   - nil 에러, 단일 에러, 래핑된 에러, 다중 래핑 테스트
   - 모든 테스트 통과

**Impact / 영향**:
- 에러 검사 기능 18개 → 21개로 확장
- 에러 디버깅 및 분석 도구 강화
- 에러 체인 탐색 기능 완성
- 테스트 커버리지 유지 (99.3%)
