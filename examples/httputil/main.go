...
// ═══════════════════════════════════════════════════════════════════════════
// EXAMPLE 2: CLIENT CONFIGURATION (Phase 1)
// 예제 2: 클라이언트 설정 (1단계)
// ═══════════════════════════════════════════════════════════════════════════
...
func example02_ClientConfiguration() {
	...
	// WithRetryBackoff - Set retry backoff strategy
	// 재시도 백오프 전략 설정
	logger.Info("")
	...
	// WithBearerToken - Set bearer token authentication
	// Bearer 토큰 인증 설정
	logger.Info("")
	...
	// WithQueryParams - Set default query parameters
	// 기본 쿼리 파라미터 설정
	logger.Info("")
	...
	// WithFollowRedirects - Control redirect behavior
	// 리다이렉트 동작 제어
	logger.Info("")
	...
	// WithMaxRedirects - Set maximum redirects
	// 최대 리다이렉트 설정
	logger.Info("")
	...
	// WithTLSConfig - Set TLS configuration
	// TLS 설정
	logger.Info("")
...
func example03_ErrorHandling() {
	...
	// IsRetryError - Check if error is retryable
	// 재시도 가능한 에러인지 확인
	logger.Info("")
...
// ═══════════════════════════════════════════════════════════════════════════
// EXAMPLE 4: RESPONSE HELPERS (Phase 2)
// 예제 4: 응답 헬퍼 (2단계)
// ═══════════════════════════════════════════════════════════════════════════
...
func example04_ResponseHelpers() {
	...
	// DoRawContext - Get raw response with context
	// Context로 원시 응답 가져오기
	logger.Info("")
	...
	// Access body in different formats
	// 다양한 형식으로 본문 접근
	logger.Info("")
...
// ═══════════════════════════════════════════════════════════════════════════
// EXAMPLE 5: FILE OPERATIONS (Phase 3)
// 예제 5: 파일 작업 (3단계)
// ═══════════════════════════════════════════════════════════════════════════
...
func example05_FileOperations() {
	...
	// Create test server for file operations
	// 파일 작업을 위한 테스트 서버 생성
	...
	// DownloadContext - Download with context
	// Context로 다운로드
	logger.Info("")
	...
	// DownloadFileContext - Download to file with progress
	// 진행 상황과 함께 파일로 다운로드
	logger.Info("")
	...
	// UploadFileContext - Upload with progress
	// 진행 상황과 함께 업로드
	logger.Info("")
	...
	// UploadFilesContext - Upload multiple files with context
	// Context로 여러 파일 업로드
	logger.Info("")
...
// ═══════════════════════════════════════════════════════════════════════════
// EXAMPLE 7: FORM BUILDER (Phase 4)
// 예제 7: Form 빌더 (4단계)
// ═══════════════════════════════════════════════════════════════════════════
...
func example07_FormBuilder() {
	...
	// Create test server for form posting
	// 폼 포스팅을 위한 테스트 서버 생성
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		r.ParseForm()
		w.WriteHeader(http.StatusOK)
	...
	// PostFormContext - Post form with context
	// Context로 폼 포스트
	logger.Info("")
...
// ═══════════════════════════════════════════════════════════════════════════
// EXAMPLE 8: COOKIE MANAGEMENT (Phase 5a)
// 예제 8: 쿠키 관리 (Phase 5a)
// ═══════════════════════════════════════════════════════════════════════════
...
func example08_CookieManagement() {
	...
	// Create test server that handles cookies
	// 쿠키를 처리하는 테스트 서버 생성
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// WithCookies - In-memory cookie jar
	// 메모리 내 쿠키 저장소
	...
	// NewCookieJar - Create in-memory cookie jar
	// 메모리 내 쿠키 저장소 생성
	logger.Info("")
	...
	// NewPersistentCookieJar - Create persistent cookie jar
	// 지속성 쿠키 저장소 생성
	logger.Info("")
	...
	// WithCookieJar - Use custom cookie jar
	// 사용자 정의 쿠키 저장소 사용
	logger.Info("")
...
func example09_AdvancedFeatures() {
	...
	// Note: Just demonstrating the API
	// API 시연만 수행
	...
	// SetDefaultClient - Set default package-level client
	// 기본 패키지 레벨 클라이언트 설정
	logger.Info("")
