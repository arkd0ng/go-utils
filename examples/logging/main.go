...
// displayHeader shows comprehensive package information for the logging examples suite / displayHeader는 로깅 예제 모음에 대한 종합 패키지 정보를 자세하게 안내합니다
func displayHeader() {
	fmt.Println("╔════════════════════════════════════════════════════════════════════════════╗")
...
// backupLogFile handles log file backup and cleanup for any log file before running examples / backupLogFile은 예제를 실행하기 전에 모든 로그 파일의 백업과 정리를 세심하게 수행합니다
...
func main() {
	...
	// Display comprehensive header before running the examples / 예제를 실행하기 전에 종합 헤더를 출력합니다
	displayHeader()

	// Example 1: Default logger - Simplest usage / 예제 1: 기본 로거 - 가장 간단한 사용법
	defaultExample()

	// Example 2: Custom logger with options / 예제 2: 옵션을 사용한 커스텀 로거
	customExample()

	// Example 3: Multiple loggers / 예제 3: 여러 로거 구성
	multipleLoggersExample()

	// Example 4: Different log levels / 예제 4: 다양한 로그 레벨
	logLevelsExample()

	// Example 5: Structured logging / 예제 5: 구조화된 로깅
	structuredLoggingExample()

	// Example 6: Auto banner examples / 예제 6: 자동 배너 사용 예제
	autoBannerExample()

	// Example 7: Manual banner examples / 예제 7: 수동 배너 사용 예제
	bannerExample()
}

// defaultExample demonstrates the simplest usage with default settings, highlighting the zero-configuration experience / defaultExample은 설정 없이도 바로 사용할 수 있는 기본 사용 예제를 자세히 보여주며 초기 경험을 강조합니다
func defaultExample() {
...
// customExample demonstrates creating a custom logger with rotation, retention, and level options / customExample은 로테이션, 보존 기간, 로그 레벨 옵션을 포함한 커스텀 로거 구성을 자세히 설명합니다
func customExample() {
...
// multipleLoggersExample demonstrates using multiple independent loggers for different subsystems / multipleLoggersExample은 서로 다른 하위 시스템을 위한 독립적인 로거 구성을 자세히 보여줍니다
...
// logLevelsExample demonstrates different log levels / logLevelsExample은 다양한 로그 레벨을 보여줍니다
...
// structuredLoggingExample demonstrates structured logging with key-value pairs / structuredLoggingExample은 키-값 쌍을 사용한 구조화된 로깅을 보여줍니다
...
// autoBannerExample demonstrates automatic banner functionality / autoBannerExample은 자동 배너 기능을 보여줍니다
func autoBannerExample() {
	// Example 6-1: Auto banner with default settings / 예제 6-1: 기본 설정으로 자동 배너 활용
	// By default, a banner is automatically printed when logger is created / 기본적으로 로거를 생성하면 배너가 자동으로 출력됩니다
	...
	// Example 6-2: Auto banner with custom app name and version / 예제 6-2: 사용자 지정 앱 이름과 버전으로 자동 배너 사용
	// You can customize the app name and version for the auto banner / 자동 배너에 표시될 앱 이름과 버전을 자유롭게 지정할 수 있습니다
	...
	// Example 6-3: Convenience function WithBanner / 예제 6-3: WithBanner 편의 함수 사용
	// Use WithBanner() to set both name and version at once / WithBanner()로 이름과 버전을 한 번에 설정할 수 있습니다
	...
	// Example 6-4: Explicitly disable auto banner / 예제 6-4: 자동 배너를 명시적으로 비활성화
	// If you don't want auto banner, disable it explicitly / 자동 배너가 필요 없다면 옵션을 통해 명시적으로 끌 수 있습니다
	...
	// Example 6-5: Disable auto banner but use manual banner / 예제 6-5: 자동 배너를 끄고 수동 배너 사용
	// You can disable auto banner and call Banner() manually when needed / 자동 배너를 끈 뒤 필요할 때 Banner()를 호출해 배너를 출력할 수 있습니다
...
// bannerExample demonstrates various banner styles to inspire customization / bannerExample은 활용 가능한 다양한 배너 스타일을 소개하여 사용자 정의 아이디어를 제공합니다
func bannerExample() {
	logger, _ := logging.New(
