...
func main() {
	// Initialize logger with detailed output / 상세한 출력으로 로거 초기화
...
func example02_FileReading(logger *logging.Logger, tempDir string) {
	...
	logger.Info("   Use Case: Small text files, configs, single-read scenarios")
	logger.Info("   사용 사례: 작은 텍스트 파일, 설정, 단일 읽기 시나리오")
	file1 := filepath.Join(exampleDir, "hello.txt")
	content, err := fileutil.ReadString(file1)
	if err != nil {
		logger.Fatalf("ReadString failed: %v", err)
	}
	logger.Info("   ✅ Read content", "length", len(content), "preview", content[:min(50, len(content))])
	logger.Info("")

	// 2. ReadFile - Read file as bytes / 파일을 바이트로 읽기
	logger.Info("2️⃣  ReadFile() - Reading file as byte array")
	logger.Info("   Use Case: Binary files, network transmission, byte processing")
	logger.Info("   사용 사례: 바이너리 파일, 네트워크 전송, 바이트 처리")
	file2 := filepath.Join(exampleDir, "bytes.bin")
	bytes, err := fileutil.ReadFile(file2)
	if err != nil {
		logger.Fatalf("ReadFile failed: %v", err)
	}
	logger.Info("   ✅ Read bytes", "count", len(bytes), "data", fmt.Sprintf("%x", bytes))
	logger.Info("")

	// 3. ReadLines - Read file as array of lines / 파일을 줄 배열로 읽기
	logger.Info("3️⃣  ReadLines() - Reading file as array of strings")
	logger.Info("   Use Case: Line-by-line processing, CSV parsing, log analysis")
	logger.Info("   사용 사례: 줄별 처리, CSV 파싱, 로그 분석")
	file3 := filepath.Join(exampleDir, "lines.txt")
	lines, err := fileutil.ReadLines(file3)
	if err != nil {
		logger.Fatalf("ReadLines failed: %v", err)
	}
	logger.Info("   ✅ Read lines", "count", len(lines))
	for i, line := range lines {
		logger.Info("      Line", "index", i+1, "content", line)
	}
	logger.Info("")

	// 4. ReadJSON - Read JSON file into struct / JSON 파일을 구조체로 읽기
	logger.Info("4️⃣  ReadJSON() - Deserializing JSON file to Go struct")
	logger.Info("   Use Case: API config, saved state, structured data")
	logger.Info("   사용 사례: API 설정, 저장된 상태, 구조화된 데이터")
	file4 := filepath.Join(exampleDir, "user.json")
	var user User
	if err := fileutil.ReadJSON(file4, &user); err != nil {
		logger.Fatalf("ReadJSON failed: %v", err)
	}
	logger.Info("   ✅ Parsed JSON", "id", user.ID, "name", user.Name, "age", user.Age)
	logger.Info("")

	// 5. ReadYAML - Read YAML file into struct / YAML 파일을 구조체로 읽기
	logger.Info("5️⃣  ReadYAML() - Deserializing YAML file to Go struct")
	logger.Info("   Use Case: Config files, deployment specs, settings")
	logger.Info("   사용 사례: 설정 파일, 배포 스펙, 설정값")
	file5 := filepath.Join(exampleDir, "user.yaml")
	var userYAML User
	if err := fileutil.ReadYAML(file5, &userYAML); err != nil {
		logger.Fatalf("ReadYAML failed: %v", err)
	}
	logger.Info("   ✅ Parsed YAML", "id", userYAML.ID, "name", userYAML.Name, "age", userYAML.Age)
	logger.Info("")

	// 6. ReadCSV - Read CSV file / CSV 파일 읽기
	logger.Info("6️⃣  ReadCSV() - Reading CSV file as 2D array")
	logger.Info("   Use Case: Excel import, data analysis, batch processing")
	logger.Info("   사용 사례: Excel 가져오기, 데이터 분석, 배치 처리")
	file6 := filepath.Join(exampleDir, "data.csv")
	csvData, err := fileutil.ReadCSV(file6)
	if err != nil {
		logger.Fatalf("ReadCSV failed: %v", err)
	}
	logger.Info("   ✅ Parsed CSV", "rows", len(csvData), "columns", len(csvData[0]))
	for i, row := range csvData {
		logger.Info("      Row", "index", i+1, "data", fmt.Sprintf("%v", row))
	}
	logger.Info("")

	logger.Info("📊 Summary: Successfully read 6 different file formats")
	logger.Info("   요약: 6가지 다른 파일 형식을 성공적으로 읽음")
	logger.Info("")
}
...
func example03_PathOperations(logger *logging.Logger, tempDir string) {
	...
	logger.Info("   Use Case: Building file paths, cross-platform compatibility")
	logger.Info("   사용 사례: 파일 경로 구축, 크로스 플랫폼 호환성")
	joined := fileutil.Join("home", "user", "documents", "file.txt")
	logger.Info("   ✅ Joined path", "result", joined)
	logger.Info("")

	// 2. Split - Split path into directory and file / 경로를 디렉토리와 파일로 분리
	logger.Info("2️⃣  Split() - Splitting path into dir and file")
	logger.Info("   Use Case: Path parsing, extracting directory")
	logger.Info("   사용 사례: 경로 파싱, 디렉토리 추출")
	dir, file := fileutil.Split(testPath)
	logger.Info("   ✅ Split result", "dir", dir, "file", file)
	logger.Info("")

	// 3. Base - Get base name / 기본 이름 가져오기
	logger.Info("3️⃣  Base() - Getting base filename")
	logger.Info("   Use Case: Extracting filename from path")
	logger.Info("   사용 사례: 경로에서 파일명 추출")
	base := fileutil.Base(testPath)
	logger.Info("   ✅ Base name", "result", base)
	logger.Info("")

	// 4. Dir - Get directory / 디렉토리 가져오기
	logger.Info("4️⃣  Dir() - Getting directory path")
	logger.Info("   Use Case: Parent directory operations")
	logger.Info("   사용 사례: 상위 디렉토리 작업")
	dirPath := fileutil.Dir(testPath)
	logger.Info("   ✅ Directory", "result", dirPath)
	logger.Info("")

	// 5. Ext - Get extension / 확장자 가져오기
	logger.Info("5️⃣  Ext() - Getting file extension")
	logger.Info("   Use Case: File type detection, filtering")
	logger.Info("   사용 사례: 파일 타입 감지, 필터링")
	ext := fileutil.Ext(testPath)
	logger.Info("   ✅ Extension", "result", ext)
	logger.Info("")

	// 6. WithoutExt - Remove extension / 확장자 제거
	logger.Info("6️⃣  WithoutExt() - Removing file extension")
	logger.Info("   Use Case: Base filename without extension")
	logger.Info("   사용 사례: 확장자 없는 기본 파일명")
	withoutExt := fileutil.WithoutExt(testPath)
	logger.Info("   ✅ Without extension", "result", withoutExt)
	logger.Info("")

	// 7. ChangeExt - Change extension / 확장자 변경
	logger.Info("7️⃣  ChangeExt() - Changing file extension")
	logger.Info("   Use Case: File conversion, format change")
	logger.Info("   사용 사례: 파일 변환, 형식 변경")
	changed := fileutil.ChangeExt(testPath, ".docx")
	logger.Info("   ✅ Changed extension", "from", ext, "to", ".docx", "result", changed)
	logger.Info("")

	// 8. HasExt - Check if has extension / 확장자 확인
	logger.Info("8️⃣  HasExt() - Checking if file has specific extension")
	logger.Info("   Use Case: File filtering, type validation")
	logger.Info("   사용 사례: 파일 필터링, 타입 검증")
	hasPdf := fileutil.HasExt(testPath, ".pdf", ".doc")
	hasDoc := fileutil.HasExt(testPath, ".txt", ".md")
	logger.Info("   ✅ Has .pdf or .doc?", "result", hasPdf)
	logger.Info("   ✅ Has .txt or .md?", "result", hasDoc)
	logger.Info("")

	// 9. Abs - Get absolute path / 절대 경로 가져오기
	logger.Info("9️⃣  Abs() - Converting to absolute path")
	logger.Info("   Use Case: Resolving relative paths, canonical paths")
	logger.Info("   사용 사례: 상대 경로 해석, 정규 경로")
	absPath, _ := fileutil.Abs(".")
	logger.Info("   ✅ Absolute path of '.'", "result", absPath)
	logger.Info("")

	// 10. IsAbs - Check if absolute / 절대 경로 확인
	logger.Info("🔟 IsAbs() - Checking if path is absolute")
	logger.Info("   Use Case: Path validation, security checks")
	logger.Info("   사용 사례: 경로 검증, 보안 체크")
	isAbs1 := fileutil.IsAbs("/home/user/file.txt")
	isAbs2 := fileutil.IsAbs("./file.txt")
	logger.Info("   ✅ '/home/user/file.txt' is absolute?", "result", isAbs1)
	logger.Info("   ✅ './file.txt' is absolute?", "result", isAbs2)
	logger.Info("")

	// 11. CleanPath - Clean path / 경로 정리
	logger.Info("1️⃣1️⃣ CleanPath() - Cleaning and normalizing path")
	logger.Info("   Use Case: Path sanitization, removing redundancies")
	logger.Info("   사용 사례: 경로 정리, 중복 제거")
	dirty := "/home/user/../user/./documents//file.txt"
	clean := fileutil.CleanPath(dirty)
	logger.Info("   ✅ Cleaned path", "before", dirty, "after", clean)
	logger.Info("")

	// 12. ToSlash & FromSlash - Path separators / 경로 구분자
	logger.Info("1️⃣2️⃣ ToSlash() / FromSlash() - Converting path separators")
	logger.Info("   Use Case: Cross-platform path handling, URL conversion")
	logger.Info("   사용 사례: 크로스 플랫폼 경로 처리, URL 변환")
	windowsPath := "C:\\Users\\John\\Documents"
	slashPath := fileutil.ToSlash(windowsPath)
	backPath := fileutil.FromSlash(slashPath)
	logger.Info("   ✅ To forward slashes", "result", slashPath)
	logger.Info("   ✅ From forward slashes", "result", backPath)
	logger.Info("")

	logger.Info("📊 Summary: Demonstrated 12 path manipulation functions")
	logger.Info("   요약: 12개의 경로 조작 함수 시연")
	logger.Info("")
}
