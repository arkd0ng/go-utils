...
func main() {
	...
	// 백업 관리와 함께 로그 파일 설정
	logFilePath := "logs/mysql-example.log"
	...
	if fileutil.Exists(logFilePath) {
		...
		if err == nil {
			...
			// 타임스탬프와 함께 백업 파일명 생성
			backupName := fmt.Sprintf("logs/mysql-example-%s.log", modTime.Format("20060102-150405"))
		...
		backupPattern := "logs/mysql-example-*.log"
	...
	// Create mysql_export directory if it doesn't exist
	// mysql_export 디렉토리 생성
	...
	if !wasRunning {
		...
		if err := waitForDockerMySQL(config.MySQL, 30*time.Second); err != nil {
			...
			// Clean up - stop MySQL if we started it
			// 정리 - 시작한 경우 MySQL 중지
			stopDockerMySQL()
			os.Exit(1)
		}
		...
		// Ensure MySQL stops when program exits
		// 프로그램 종료 시 MySQL 중지 보장
		defer func() {
			logger.Info("")
...
func initializeDatabaseIfNeeded(dsn string, logger *logging.Logger) error {
	...
	// Check if sample data exists by looking for a known sample user
	// 알려진 샘플 사용자를 찾아 샘플 데이터 존재 확인
...
func example1SelectAll(_ context.Context, db *mysql.Client, logger *logging.Logger) error {
	...
	// Select all users from Seoul
	// 서울의 모든 사용자 선택
	// Using non-context version for simplicity
	// 간단함을 위해 non-context 버전 사용
...
func example8Delete(_ context.Context, db *mysql.Client, logger *logging.Logger) error {
	...
	// Delete Charlie Brown (one of the sample users)
	// 샘플 사용자 중 한 명인 Charlie Brown 삭제
...
func example20BatchDelete(ctx context.Context, db *mysql.Client, logger *logging.Logger) error {
	...
	// First, get some user IDs to delete
	// 먼저 삭제할 사용자 ID 가져오기
...
func example24PaginationWithOptions(ctx context.Context, db *mysql.Client, logger *logging.Logger) error {
	...
	// Get page 1 of users older than 25, ordered by age
	// 25세 이상 사용자의 1페이지, 나이순 정렬
