...
// TestShutdownNotRunning tests Shutdown when server is not running
// 서버가 실행 중이 아닐 때 Shutdown 테스트
...
// TestShutdownWithTimeout tests shutdown timeout
// 종료 타임아웃 테스트
...
// TestRunWithGracefulShutdown tests RunWithGracefulShutdown method
// RunWithGracefulShutdown 메서드 테스트
func TestRunWithGracefulShutdown(t *testing.T) {
	...
	// Run server with graceful shutdown in background
	// 백그라운드에서 graceful shutdown과 함께 서버 실행
...
func TestGracefulShutdownWithActiveConnections(t *testing.T) {
	...
	// 최소 하나의 요청이 완료되었는지 확인
	select {
	case <-requestCompleted:
		// Request completed successfully
		// 요청이 성공적으로 완료됨
	case <-time.After(1 * time.Second):
		// OK if no requests completed due to shutdown
		// 종료로 인해 완료된 요청이 없어도 OK
	}
}

// TestShutdownIdempotent tests that multiple shutdown calls are safe
// 여러 번의 종료 호출이 안전한지 테스트
func TestShutdownIdempotent(t *testing.T) {
	...
	// Second shutdown (should return error since server is not running)
	// 두 번째 종료 (서버가 실행 중이 아니므로 에러 반환해야 함)
...
// BenchmarkShutdown benchmarks the Shutdown method
// Shutdown 메서드 벤치마크
