...
// TestStaticFileServing tests the Static method / Static 메서드 테스트
func TestStaticFileServing(t *testing.T) {
	// Create temporary directory with test files / 테스트 파일이 있는 임시 디렉토리 생성
	tmpDir := t.TempDir()
	...
	// Create app with static route / 정적 라우트가 있는 앱 생성
	app := New()
	app.Static("/static", tmpDir)

	// Create server to handle redirects / 리디렉트를 처리하는 서버 생성
	server := httptest.NewServer(app)
	defer server.Close()

	// Test index.html / index.html 테스트
	...
	// Test subdirectory file / 하위 디렉토리 파일 테스트
...
// TestStaticNotFound tests 404 for non-existent static files / 존재하지 않는 정적 파일에 대한 404 테스트
func TestStaticNotFound(t *testing.T) {
	...
	// Test non-existent file / 존재하지 않는 파일 테스트
...
// TestStaticMultiplePrefixes tests multiple static directories / 여러 정적 디렉토리 테스트
func TestStaticMultiplePrefixes(t *testing.T) {
	// Create two temporary directories / 두 개의 임시 디렉토리 생성
	tmpDir1 := t.TempDir()
	tmpDir2 := t.TempDir()

	// Create test files in each directory / 각 디렉토리에 테스트 파일 생성
	...
	// Test first static directory / 첫 번째 정적 디렉토리 테스트
	...
	// Test second static directory / 두 번째 정적 디렉토리 테스트
...
// TestFile tests the File method / File 메서드 테스트
func TestFile(t *testing.T) {
	// Create temporary test file / 임시 테스트 파일 생성
	tmpDir := t.TempDir()
	...
	// Check Content-Type header / Content-Type 헤더 확인
...
// TestFileNotFound tests File with non-existent file / 존재하지 않는 파일로 File 테스트
...
// TestFileAttachment tests the FileAttachment method / FileAttachment 메서드 테스트
func TestFileAttachment(t *testing.T) {
	...
	// Check Content-Disposition header / Content-Disposition 헤더 확인
...
// TestFileAttachmentDifferentTypes tests FileAttachment with various file types / 다양한 파일 타입으로 FileAttachment 테스트
...
// BenchmarkStaticFileServing benchmarks static file serving / 정적 파일 서빙 벤치마크
...
// BenchmarkFile benchmarks the File method / File 메서드 벤치마크
