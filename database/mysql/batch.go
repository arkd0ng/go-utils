...
func (c *Client) BatchInsert(ctx context.Context, table string, data []map[string]interface{}) (sql.Result, error) {
	...
	// Capture the column order from the first row so every placeholder array aligns with the same ordering. / 첫 번째 행에서 컬럼 순서를 추출하여, 이후 플레이스홀더 배열이 항상 동일한 순서를 따르도록 보장합니다.
	var columns []string
	...
	// Render the comma-separated column projection used in the INSERT clause / INSERT 절에서 사용할 콤마 구분 컬럼 목록을 생성합니다.
	columnList := strings.Join(columns, ", ")

	// Prepare a single parenthesised placeholder sequence such as "(?,?,?)". / "(?,?,?)"와 같이 괄호로 감싼 플레이스홀더 시퀀스를 준비합니다.
	valuePlaceholder := "(" + strings.Repeat("?,", len(columns))
	valuePlaceholder = valuePlaceholder[:len(valuePlaceholder)-1] + ")"

	// Duplicate the placeholder block for every row so that all values can be supplied in a single Exec call / 단일 Exec 호출로 모든 값을 전달할 수 있도록 행 수만큼 플레이스홀더 블록을 복제합니다.
	var valuePlaceholders []string
	for i := 0; i < len(data); i++ {
		valuePlaceholders = append(valuePlaceholders, valuePlaceholder)
	}

	// Combine the projection and placeholder list into the final INSERT statement / 컬럼 목록과 플레이스홀더 목록을 결합하여 최종 INSERT 문을 구성합니다.
	query := fmt.Sprintf("INSERT INTO %s (%s) VALUES %s",
		table, columnList, strings.Join(valuePlaceholders, ","))

	// Flatten every map value into a contiguous args slice that matches the generated placeholders. / 각 행의 값을 생성된 플레이스홀더 순서와 일치하도록 하나의 연속된 슬라이스로 평탄화합니다.
	var args []interface{}
	...
	// Execute the statement using the shared retry policy to absorb transient network or lock issues. / 네트워크 지연이나 잠금과 같은 일시적 문제를 흡수하기 위해 공유 재시도 정책을 사용해 구문을 실행합니다.
...
func (c *Client) BatchUpdate(ctx context.Context, table string, updates []BatchUpdateItem) error {
	...
	// Run the per-row updates within a transaction so that either all succeed or none are applied. / 각 행에 대한 업데이트를 트랜잭션으로 감싸 모든 변경이 함께 성공하거나 전부 롤백되도록 합니다.
...
func (c *Client) BatchDelete(ctx context.Context, table string, idColumn string, ids []interface{}) (sql.Result, error) {
	...
	// Construct the "?,?,?" placeholder list matching the number of identifiers supplied. / 전달된 식별자 개수에 맞는 "?,?,?" 형태의 플레이스홀더 목록을 구성합니다.
	placeholders := strings.Repeat("?,", len(ids))
	// Trim the trailing comma that Repeat leaves at the end. / Repeat 호출이 남긴 마지막 쉼표를 제거합니다.
	placeholders = placeholders[:len(placeholders)-1]

	// Build the DELETE query using the generated placeholder list. / 생성된 플레이스홀더 목록을 사용해 DELETE 쿼리를 완성합니다.
	query := fmt.Sprintf("DELETE FROM %s WHERE %s IN (%s)", table, idColumn, placeholders)

	start := time.Now()

	// Execute the statement with the same retry semantics used for inserts. / INSERT와 동일한 재시도 규칙으로 구문을 실행합니다.
	var result sql.Result
	err := c.executeWithRetry(ctx, func() error {
		db := c.getCurrentConnection()
		var execErr error
...
func (c *Client) BatchSelectByIDs(ctx context.Context, table string, idColumn string, ids []interface{}) ([]map[string]interface{}, error) {
	...
	// Build the placeholder sequence that mirrors the number of IDs passed in. / 전달된 ID 개수와 동일한 플레이스홀더 시퀀스를 생성합니다.
	placeholders := strings.Repeat("?,", len(ids))
	// Remove the extra comma introduced by Repeat so the SQL remains valid. / Repeat로 인해 추가된 마지막 쉼표를 제거하여 올바른 SQL을 유지합니다.
	placeholders = placeholders[:len(placeholders)-1]

	// Assemble the SELECT statement that filters by the provided identifiers. / 전달받은 식별자에 따라 필터링하는 SELECT 문을 구성합니다.
	query := fmt.Sprintf("SELECT * FROM %s WHERE %s IN (%s)", table, idColumn, placeholders)

	start := time.Now()

	// Issue the query using the shared retry wrapper to guard against transient errors. / 일시적 오류에 대비하기 위해 재시도 래퍼를 사용하여 쿼리를 실행합니다.
	var rows *sql.Rows
	err := c.executeWithRetry(ctx, func() error {
		db := c.getCurrentConnection()
		var execErr error
	...
	// Convert the sql.Rows cursor into a slice of map[string]interface{} for ergonomic consumption. / sql.Rows 커서를 map[string]interface{} 슬라이스로 변환하여 사용성을 높입니다.
