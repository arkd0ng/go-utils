...
// TestEvery tests the Every function / Every 함수 테스트
func TestEvery(t *testing.T) {
	data := map[string]int{"a": 2, "b": 4, "c": 6}

	// All values are even / 모든 값이 짝수
	...
	// Not all values > 3 / 모든 값이 3보다 큰 것은 아님
	result = Every(data, func(k string, v int) bool {
		return v > 3
	})
	if result {
	...
	// Empty map should return true / 빈 맵은 true 반환
...
// TestSome tests the Some function / Some 함수 테스트
func TestSome(t *testing.T) {
	data := map[string]int{"a": 1, "b": 2, "c": 3}

	// At least one even value / 최소 한 개의 짝수 값
	...
	// Empty map should return false / 빈 맵은 false 반환
	empty := map[string]int{}
...
// TestNone tests the None function / None 함수 테스트
func TestNone(t *testing.T) {
	data := map[string]int{"a": 1, "b": 3, "c": 5}

	// No even values / 짝수 값 없음
...
// TestHasValue tests the HasValue function / HasValue 함수 테스트
...
// TestHasEntry tests the HasEntry function / HasEntry 함수 테스트
func TestHasEntry(t *testing.T) {
	...
	// Key exists but value doesn't match / 키는 있지만 값이 다름
	if HasEntry(data, "b", 99) {
		t.Error("HasEntry('b', 99) = true, want false")
	}

	// Key doesn't exist / 키가 없음
	if HasEntry(data, "x", 1) {
		t.Error("HasEntry('x', 1) = true, want false")
	}
}

// TestIsSubset tests the IsSubset function / IsSubset 함수 테스트
...
func TestIsSuperset(t *testing.T) {
	...
	// Any map is superset of empty map / 모든 맵은 빈 맵의 상위집합
	empty := map[string]int{}
