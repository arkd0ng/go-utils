...
func ParseAny(s string) (time.Time, error) {
	...
	// All formats to try / 시도할 모든 포맷
	formats := []string{
		// ISO8601 and RFC3339 / ISO8601 및 RFC3339
		time.RFC3339,
		time.RFC3339Nano,
		ISO8601Layout,
		...
		// DateTime with sub-seconds / 밀리초/마이크로초/나노초 포함
		...
		// Standard Go time formats / Go 표준 시간 포맷
		time.ANSIC,
		time.UnixDate,
		time.RubyDate,
		time.RFC822,
		...
		// Additional common formats / 추가 일반 포맷
		"2006-01-02 15:04:05 MST",
		"2006-01-02 15:04:05 -0700",
		"Mon, 02 Jan 2006 15:04:05 -0700",
	}

	// Try parsing with each format / 각 포맷으로 파싱 시도
	for _, layout := range formats {
		// Try with default location first / 기본 타임존으로 먼저 시도
		t, err := time.ParseInLocation(layout, s, defaultLocation)
		if err == nil {
			return t, nil
		}

		// Try without location / 타임존 없이 시도
